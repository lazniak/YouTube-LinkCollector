...
interface TopicSearcherFormProps {
  onSearch: (query: string, count: number, options: SearchOptions) => void;
  isLoading: boolean;
}

export const TopicSearcherForm: React.FC<TopicSearcherFormProps> = ({ onSearch, isLoading }) => {
    const [query, setQuery] = useState('');
    const [count, setCount] = useState(300);
    const [order, setOrder] = useState<SearchOptions['order']>('relevance');
    const [duration, setDuration] = useState<SearchOptions['videoDuration']>('any');

    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        onSearch(query, count, { order, videoDuration: duration });
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-grow group">
                    ...
                    <input
                        ...
                        placeholder="Szukaj tematu (np. postępy w AI)"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label htmlFor="video-count" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Ilość (Maks. 1000)</label>
                    <input
                        id="video-count"
                        type="number"
                        value={count}
                ...
                <div>
                    <label htmlFor="order" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Sortuj według</label>
                    <div className="relative">
                        ...
                        >
                            ...
                            <option value="viewCount">Liczba wyświetleń</option>
                ...
                <div>
                    <label htmlFor="duration" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Długość</label>
                    <div className="relative">
                        <select
                            id="duration"
                            value={duration}
